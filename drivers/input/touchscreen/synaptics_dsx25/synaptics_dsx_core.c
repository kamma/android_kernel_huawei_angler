...
static int synaptics_rmi4_set_gpio(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (retval < 0) {
		...
		tp_log_debug("%s: irq_gpio(%d) setup !\n",__func__,bdata->irq_gpio);
	}
	...
	if (bdata->power_gpio >= 0) {
		...
		if (retval < 0) {
			...
			tp_log_debug("%s: power_gpio(%d) setup !\n",__func__,bdata->power_gpio);
		}
	}
	...
	if (bdata->reset_gpio >= 0) {
		...
		if (retval < 0) {
			...
			tp_log_debug("%s: reset_gpio(%d) setup !\n",__func__,bdata->reset_gpio);
		}
	}

	if (bdata->power_gpio >= 0) {
		gpio_set_value(bdata->power_gpio, bdata->power_on_state);
		tp_log_debug("%s: power_gpio(%d) on !\n",__func__,bdata->power_gpio);
		msleep(bdata->power_delay_ms);
	}

	if (bdata->reset_gpio >= 0) {
		tp_log_debug("%s: reset_gpio(%d) on=%d !\n",__func__,bdata->reset_gpio,bdata->reset_on_state);
		gpio_set_value(bdata->reset_gpio, bdata->reset_on_state);
		msleep(bdata->reset_active_ms);
		tp_log_debug("%s: reset_active_ms(%d) !\n",__func__,bdata->reset_active_ms);
		gpio_set_value(bdata->reset_gpio, !bdata->reset_on_state);
		msleep(bdata->reset_delay_ms);
		tp_log_debug("%s: reset_delay_ms(%d) !\n",__func__,bdata->reset_delay_ms);
	}
		...
		bool get)
{
	...
	if ((bdata->pwr_reg_name != NULL) && (*bdata->pwr_reg_name != 0)) {
		...
		if (IS_ERR(rmi4_data->pwr_reg)) {
			...
			tp_log_debug("%s: pwr_reg get !\n",__func__);
		}

		if (regulator_count_voltages(rmi4_data->pwr_reg) > 0) {
			retval = regulator_set_voltage(rmi4_data->pwr_reg,
				DSX_VDD_VTG_MIN, DSX_VDD_VTG_MAX);
			if (retval) {
				dev_err(rmi4_data->pdev->dev.parent,
					"regulator set pwr_reg(Vdd) failed retval =%d\n",
					retval);
				goto regulator_put;
			}
		}
...
void synaptics_dsx_new_function(struct synaptics_rmi4_exp_fn *exp_fn,
		bool insert)
{
	struct synaptics_rmi4_exp_fhandler *exp_fhandler;
...
EXPORT_SYMBOL(synaptics_dsx_new_function);
		...
		unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && rmi4_data) {
		if (event == FB_EVENT_BLANK) {
			tp_log_debug("%s: event = FB_EVENT_BLANK!\n",__func__);
